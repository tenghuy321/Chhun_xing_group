...
@section('content')
    <div class="">
        <div class="my-4 px-2 md:px-4 text-end">
            {{-- <a href="{{ route('special_project.create') }}"
            ...
            <div class="border border-[#966927] max-h-[80vh] overflow-y-auto">
                <table class="min-w-full table-fixed">
                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($special_projects as $index => $special_project)
                            <tr class="border-t border-[#966927]">
                                ...
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <img src="{{ asset($special_project->icon) }}" alt="" class="w-10 h-10">
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <div class="flex flex-col truncate">
                                        <p>{{ $special_project->title['en'] ?? '' }}</p>
                                        <p>{{ $special_project->title['kh'] ?? '' }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px]">
                                    <div class="flex items-center space-x-1">
                                        <a href="{{ route('special_project.edit', $special_project->id) }}" title="Edit">
