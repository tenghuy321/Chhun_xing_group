    ...
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('aboutlist.update', $aboutlist->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @if (!empty($aboutlist->icon))
                <div>
                    <label for="dropzone-file{{ $aboutlist->id }}" id="drop-area"
                        class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-400">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($aboutlist->icon) }})">
                        ...
                        <input id="dropzone-file{{ $aboutlist->id }}" type="file" class="hidden" name="icon"
