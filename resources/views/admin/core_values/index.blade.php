                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($core_values as $index => $core_value)
                            <tr class="border-t border-[#966927] cursor-move" draggable="true"
                                x-bind:data-id="{{ $core_value->id }}" @dragstart="dragStart($event, {{ $core_value->id }})"
                                @dragover.prevent @drop="drop($event, {{ $core_value->id }})">
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">{{ $core_value->order }}
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <div class="flex flex-col truncate">
                                        <p>{{ $core_value->title['en'] ?? '' }}</p>
                                        <p>{{ $core_value->title['kh'] ?? '' }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <div class="flex flex-col">
                                        <div class="line-clamp-1">
                                            {!! $core_value->content['en'] ?? '' !!}
                                        </div>
                                        <div class="line-clamp-1">
                                            {!! $core_value->content['kh'] ?? '' !!}
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3 px-4 text-xs max-w-[200px]">
                                    <div class="flex space-x-1 items-center">
                                        <a href="{{ route('core_value.delete', $core_value->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('core_value.delete', $core_value->id) }}')">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('core_value.reorder') }}", {
                                    method: "POST",
                                    headers: {
