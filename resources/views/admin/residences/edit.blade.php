<x-app-layout>
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Chhun Xing Residential Project Detail</h2>
        <form action="{{ route('residence.update', $residence->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('name.en', $residence->name['en'] ?? '') }}" type="text" name="name[en]"
                    ...
                    <div>
                        ...
                        <textarea name="content[en]" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content.en', $residence->content['en'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('name.kh', $residence->name['kh'] ?? '') }}" type="text" name="name[kh]"
                    ...
                    <div>
                        ...
                        <textarea name="content[kh]" id="content_kh" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content.kh', $residence->content['kh'] ?? '') }}</textarea>
            ...
            <!-- Dropzone for Image -->
            <div>
                <label for="dropzone-file{{ $residence->id }}" id="drop-area"
                    class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">

                    <input id="dropzone-file{{ $residence->id }}" type="file" class="hidden" name="images[]"
    ...
    <script>
        ...
        const fileInput = document.getElementById("dropzone-file{{ $residence->id }}");
