                ...
                <table class="min-w-full table-fixed">
                    <thead class="text-black sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 text-xs border-r border-[#966927] w-[200px]">Residence Name</th>
                            <th class="text-left py-3 px-4 text-xs border-r border-[#966927] w-[200px]">Residence Content</th>
                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($residences as $index => $residence)
                            <tr class="border-t border-[#966927] cursor-move" draggable="true"
                                x-bind:data-id="{{ $residence->id }}" @dragstart="dragStart($event, {{ $residence->id }})"
                                @dragover.prevent @drop="drop($event, {{ $residence->id }})">
                                ...
                                <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#966927]">
                                    <div class="flex items-center h-full w-full">
                                        @php
                                            $images = json_decode($residence->image, true); // decode to array
                                        @endphp
                                ...
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <div class="flex flex-col truncate">
                                        <p>{{ $residence->name['en'] ?? '' }}</p>
                                        <p>{{ $residence->name['kh'] ?? '' }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#966927]">
                                    <div class="flex flex-col">
                                        <div class="prose line-clamp-1">
                                            {!! $residence->content['en'] ?? '' !!}
                                        </div>
                                        <div class="prose line-clamp-1">
                                            {!! $residence->content['kh'] ?? '' !!}
                                        </div>
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs border-r border-[#966927]">
                                    <div class="flex items-center space-x-1">
                                        <a href="{{ route('residence.delete', $residence->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('residence.delete', $residence->id) }}')">
                                        ...
                                        <a href="{{ route('residence.edit', $residence->id) }}" title="Edit">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('residence.reorder') }}", {
                                    method: "POST",
                                    headers: {
