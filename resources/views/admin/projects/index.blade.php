                ...
                <table class="min-w-full table-fixed">
                    <thead class="text-black sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 text-xs border-r border-[#966927] w-[200px]">Project Content</th>
                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($projects as $index => $project)
                            <tr class="border-t border-[#966927] cursor-move" draggable="true"
                                x-bind:data-id="{{ $project->id }}" @dragstart="dragStart($event, {{ $project->id }})"
                                @dragover.prevent @drop="drop($event, {{ $project->id }})">
                                ...
                                <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#966927]">
                                    <div class="flex items-center h-full w-full">
                                        @php
                                            $images = json_decode($project->image, true); // decode to array
                                        @endphp
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('project.reorder') }}", {
                                    method: "POST",
                                    headers: {
