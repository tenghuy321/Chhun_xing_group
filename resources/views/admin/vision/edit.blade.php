<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Vision</h2>
        <form action="{{ route('vision_page.update', $vision->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="content[en]" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content.en', $vision->content['en'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="content[kh]" id="content_kh" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-black text-[12px]">{{ old('content.kh', $vision->content['kh'] ?? '') }}</textarea>
