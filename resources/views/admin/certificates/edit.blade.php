<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Certificate</h2>
        <form action="{{ route('certificate.update', $certificate->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <!-- Dropzone for Image -->
            <div>
                <label for="dropzone-file{{ $certificate->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url({{ asset($certificate->image) }})">
                    ...
                    <input id="dropzone-file{{ $certificate->id }}" type="file" class="hidden" name="image"
